...
- `frontend/`: React 19 + TypeScript frontend (UI components, hooks, typed Tauri invoke wrappers, Zustand store).
- `frontend/components/ui/`: shadcn/ui primitives; regenerate via `pnpm dlx shadcn@latest`, don't hand‑edit.
- `backend/src/`: Rust backend for Tauri (AI agent system, PTY/terminal, sidecar context capture, settings, headless CLI).
...
- TypeScript/React: prefer functional components; hooks live in `frontend/hooks/`; global state in `frontend/store/`.
...
- Rust: `cargo test` runs inline `#[cfg(test)]` modules under `backend/src/`.
